/* ==========================================================================
   ...
   does nothing and the hand-written fallback cards already in index.html
   stay exactly as they are — nothing is ever left blank.

   IMPORTANT: this page is actually rendered by a React runtime
   (support.js). #rw-work-grid is a React-managed DOM node, so calling
   this from a plain deferred <script> tag doesn't work — React's own
   initial mount runs shortly after and silently resets the node back to
   its original static markup, with no visible error. This is why the
   function below is exposed on window instead of auto-running: it must
   be called from componentDidMount() inside index.html's own
   <script type="text/x-dc" data-dc-script> block, which runs AFTER
   React has mounted, so the mutation sticks for good.
...
window.RWRenderFeaturedWork = function () {
  "use strict";
