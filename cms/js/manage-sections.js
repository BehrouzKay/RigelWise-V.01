...
(function () {
  ...
  generateBtn.addEventListener("click", function () {
    ...
    if (portfolioUrls.length) {
      // Small delay so the browser doesn't drop the second automatic download.
      setTimeout(function () { downloadJson("portfolio.json", portfolioUrls); }, 300);
      downloaded.push("portfolio.json");
    }

    showToast(
      downloaded.length
        ? "Downloaded " + downloaded.join(" and ") + " — drop them into the project root, next to index.html."
